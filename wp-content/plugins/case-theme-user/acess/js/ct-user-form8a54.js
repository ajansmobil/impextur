jQuery(document).ready(function ($) {
  ...
  /* enter key / input. */
  $(document)
    .find(".ct-user-form-login .login-group")
  ...
  /* register login */
  $(document)
    .find(".ct-user-form-register")
    .on("click", ".btn-up-register", function (e) {
      e.preventDefault();
  ...
  function user_press_register(e) {
    ...
    var register_data = register_validation(
      input_user,
      input_pass,
      input_passconfirm,
      input_email,
  ...
  function user_press_login(e) {
    ...
    /* validate user name. */
    if (input_user_val == "") wl_alert(input_user, input_user.data("validate"));

    /* validate password. */
    if (input_pass_val == "") wl_alert(input_pass, input_pass.data("validate"));
  ...
  function wl_alert(obj, message, mode) {
    ...
    if (mode == "show") {
      if (obj.next().hasClass("wpcf7-not-valid-tip") === false) {
        obj.after(
          '<span role="alert" class="wpcf7-not-valid-tip" style="display:none">' +
            message +
            "</span>",
        );
